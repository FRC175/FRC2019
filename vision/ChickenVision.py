...
# Updated to (hopefully) work on Kangaroo by Buzz 175
#----------------------------------------------------------------------------
...
def findTape(contours, image, centerX, centerY):
    ...
    if len(contours) >= 2:
        ...
        for i in range(len(biggestCnts) - 1):
            ...
            if (np.sign(tilt1) != np.sign(tilt2)):
                ...
                ### PITCH STUFF - NEED TO TEST!!! ###
                vertCenterOfTarget = math.floor((cy1 + cy2) / 2)
                pitchToTarget = calculatePitch(vertCenterOfTarget, centerY, V_FOCIAL_LENGTH)

                if not distances:
                    targets.append([vertCenterOfTarget, pitchToTarget])
                elif [vertCenterOfTarget, pitchToTarget] not in distances:
                    targets.append([vertCenterOfTarget, pitchToTarget])
    ...
    if (len(targets) > 0):
        ...
        ### DISTANCE STUFF - NEED TO TEST!!! ###
        distances.sort(key=lambda y: math.fabs(y[0]))
        finalDistance = min(targets, key=lambda y: math.fabs(y[1]))
        # Calculate wanted pitch and distance from centerOfTarget
        wantedPitch = calculatePitch(cameraHeight, targetHeight, finalDistance[1])
        # Push distance from target to network tables
        networkTable.putNumber("distanceFromTape", finalDistance)
        networkTable.putNumber("tapePitch", wantedPitch)
...
### REMOVE RPI STUFF TO WORK ON KANGAROO - NEED TO TEST!!! ###
